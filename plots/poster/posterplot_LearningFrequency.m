...
function  posterplot_LearningFrequency(Rf,Lf,Rfu,Lfu)
    if nargin<4, error('Not enough input arguments'); end
    
    %Define or fetch some globals
    [grp, ss, idl, idr, reps]=size(Rf);    
    plot_type='figure';
    hands=2;
        ...
        for r=1:idr
            for l=1:idl     
                ...
                if g==3
                    ...
                    for s=1:ss
                        bar_grps(1,ord,cnt,s,1)=mean(Lfu(2,s,l,:));
                        bar_grps(1,ord,cnt,s,2)=std(Lfu(2,s,l,:));
                        bar_grps(2,ord,cnt,s,1)=mean(Rfu(2,s,r,:));
                        bar_grps(2,ord,cnt,s,2)=std(Rfu(2,s,r,:));
                    end
                else                    
                    for s=1:ss
                        bar_grps(1,ord,cnt,s,1)=mean(Lf(g,s,l,r,:));
                        bar_grps(1,ord,cnt,s,2)=std(Lf(g,s,l,r,:));       
                        bar_grps(2,ord,cnt,s,1)=mean(Rf(g,s,l,r,:));
                        bar_grps(2,ord,cnt,s,2)=std(Rf(g,s,l,r,:));
                    end            
                end
            end
        end
    ...
    for hand=1:hands
        ...
        for g=1:grp+1            
            ...
            set(gca,'xlim',[0,9]);
            set(gca,'box','off');
            %if hand==2, set(gca, 'YDir', 'reverse'); end
        ...
        hold off
        %fn=['freqs',num2str(hand)];
        %print( gcf, '-deps', '-r600', '-loose',fn )
    end
