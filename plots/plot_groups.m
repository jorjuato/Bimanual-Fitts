...
function  plot_groups_Frequency(bi,un,biNames,unNames)
    ...
    %Fetch all data for bars
    %DOMINTANT RIGHT FREQUENCY
    v1=strcmp('Rf', biNames);
    %v2=strcmp('Rf', unNames);
    FRb      = squeeze(bi(v1,:,:,:,:,:,:));
    %FRun     = squeeze(un(v2 ,:,:,:,:,:));

    %DOMINANT LEFT FREQUENCY
    v3=strcmp('Lf', biNames);
    %v4=strcmp('Lf', unNames);
    FLb      = squeeze(bi(v3,:,:,:,:,:,:));
    %FLun     = squeeze(un(v4 ,:,:,:,:,:));
    
    bar_grps = get_bar_groups({FLb,FRb},data_series); 
    plot_bars_grouped(bar_grps,data_series,plot_type,{'Lf','Rf'});
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function  plot_groups_Phase(bi,biNames)
    ...
    %Fetch all data for bars
    vnames={'phDiffMean','phDiffStd','MI'};
    vars={};
