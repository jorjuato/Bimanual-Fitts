function [P,B,PC]=prob_2D(obj,x,b,step,minValsToComputeCondProb)

% Compute the probability of p[x(t)] and
% the conditional probability p[x(t+step)|x(t)] of a stochastic process.
%
% input:  - x contains 2 random sequence(s) of length N (Nx2 array)
...
if nargin>2 & nargout>2
   for k1=1:M(1)
      for k2=1:M(2)
         ii=intersect(indices{k1,1},indices{k2,2});
         ii=ii(:);
         P(k1,k2)=length(ii);
         ii=ii+step;
         ii=ii(ii<=size(x,1));
         if length(ii)>minValsToComputeCondProb
            PC(:,:,k1,k2)=obj.prob_2D(x(ii,:),b);
         end
      end
   end
else
