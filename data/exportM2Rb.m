function exportM2Rb(mfile,filename,savepath)
    ...
    [header,vnames2]=build_headers({'grp' 'pp' 'S' 'DID'},obj.varnamesB,obj.vartypesB);
    out = get_rowdata(obj.dataD,obj.vtypesB,obj.varnamesB,vnames2,header);  
    ...
    out = get_rowdata(obj.dataU,obj.vtypesU,obj.vnamesU,vnames2,header);  
    export_matrix(out,header,joinpath(savepath,['Uni_L' filename]));
    out = get_rowdata(obj.dataU,obj.vtypesU,obj.vnamesU,vnames2,header);  
    export_matrix(out,header,joinpath(savepath,['Uni_R' filename]));
end
