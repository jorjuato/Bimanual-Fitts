function [D,xo,names]=KMcoef_3D(xo,pc,dt,ord)
%[D,x,names]=KMcoef_3D(xo,pc,dt,ord)
%Computation of Kramers-Moyal coefficients
%
% input:  - xo bin centers of the cond. probability distribution
%         - pc conditional probability distribution (see prob_3D)
...
for k=1:length(ord)
   for n1=ord(k):-1:0
      for n2=ord(k):-1:0
         for n3=ord(k):-1:0
            if sum([ n1 n2 n3 ])==ord(k), names{j}=[ n1 n2 n3 ]; j=j+1; end
         end
      end
   end
end
...
for k=1:length(names)
   ...
   if names{k}(3)==1
      for i=1:length(xo{3})
         for j=1:length(xo{3})
            pcc(:,:,i,:,:,j)=pcc(:,:,i,:,:,j)*(xo{3}(i)-xo{3}(j));
         end
      end
   elseif names{k}(3)
      for i=1:length(xo{3})
         for j=1:length(xo{3})
            pcc(:,:,i,:,:,j)=pcc(:,:,i,:,:,j)*(xo{3}(i)-xo{3}(j))^(names{k}(3));
         end
      end
   end
   for n=1:3
