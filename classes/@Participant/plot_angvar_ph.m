function plot_angvar_ph(obj,vname,graphPath)
    ...
    for i=1:IDL
        for j=1:IDR
            %fprintf('Plotting %s hist figure L=%d:R=%d',vname,i,j)
            plot_angvar_phhist(obj,vname,i,j,graphPath);
            %fprintf('Plotting %s ts figure L=%d:R=%d',vname,i,j)
            %plot_angvar_phts(obj,i,j,graphPath);
        end
    end
end

function plot_angvar_phhist(obj,vname,l,r,graphPath)
    lnames={'Easy','Diff'};
    ...
    ylimits=obj.get_ylimits_ph(l,r,vname);
    v=[vname,'_hist'];
    ...
    for n=1:3
        ...
        for rep=1:3
            scatter(DSl{l,rep}.ts.ph_hist,DSl{l,rep}.ts.(v),colors{rep});
        end
        ...
        for rep=1:3
            scatter(DSb{l,r,rep}.ts.Lph_hist,DSb{l,r,rep}.ts.(['L',v]),colors{rep});
        end
        ylim(ax,ylimits(2,:));
        xlim(ax,[-pi,pi]);        
        ...
        for rep=1:3
            scatter(DSb{l,r,rep}.ts.Rph_hist,DSb{l,r,rep}.ts.(['R',v]),colors{rep});
        end
        ylim(ax,ylimits(3,:));
        xlim(ax,[-pi,pi]);        
        ...
        for rep=1:3
            scatter(DSr{r,rep}.ts.ph_hist,DSr{r,rep}.ts.(v),colors{rep});
        end
        ylim(ax,ylimits(4,:));
        xlim(ax,[-pi,pi]);        
