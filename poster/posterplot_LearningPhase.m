...
function  posterplot_LearningPhase(phDMean,phDSTD,phDMI)
    if nargin<3, error('Not enough input arguments'); end
    
    %Define or fetch some globals
    [grp, ss, idl, idr, reps]=size(phDMean);
    plot_type='figure';
    vars={phDMean,phDSTD,phDMI};
    bar_widths=[1,0.6];
    do_legend=1;
    rootname='Phase';
    ...
    for v=1:length(vars)
        ...
        elseif ~isempty(strfind(plot_type,'figure'))
            figure('Position',[1 scrsz(4)/2 scrsz(3)/1 scrsz(4)/3])
            if v==1
                figname=[rootname,'_phaseDiffMean'];
            elseif v==2
                figname=[rootname,'_phaseDiffSTD'];
            elseif v==3
                figname=[rootname,'_phaseDiffMI'];
            end
