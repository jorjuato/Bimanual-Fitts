...
do.summary.did <- function(mdata,vname,vpath){
    ...
    print("Summary by DID\n")
    form<-as.formula(paste(vname,"~ DID"))
    print(summaryBy(form, mdata,FUN=statfcn))
    ...
    print("Summary by Session + DID\n")
    form<-as.formula(paste(vname,"~ S + DID"))
    print(summaryBy(form, mdata,FUN=statfcn))
    
    print("Summary by group + DID\n")
    form<-as.formula(paste(vname,"~ grp + DID"))
    print(summaryBy(form, mdata,FUN=statfcn))
    
    print("Summary by group + Session + DID\n")
...
do.aov.did <- function(mdata,vname,vpath){
    #Repeated Measures 5-WAY ANOVA with mixed within and between design, Type III SSE
    form<-sprintf('aov.mod<-aov( %s ~ S*DID*grp + Error(pp/(S*DID)), mdata)', vname)
...
do.lme.did <- function(mdata,vname,vpath){
    #Perform lme    
    form<-sprintf('lme.mod<-lme(%s~S+DID+grp, random = ~ 1 | pp,mdata)',vname)
    ...
    print("DID Tukey test")
    print(summary(glht(lme.mod,linfct=mcp(DID= "Tukey"))))
    sink()
}

do.ANOVA.did <- function(mdata,vname,vpath){
    #Perform type III Repeated Measures 5-WAY ANOVA with mixed within and between design
    form<-sprintf('ez.mod = ezANOVA(data=mdata,dv=.(%s),wid=.(pp), within=.(S,DID),between=.(grp),type=3, return_aov=FALSE)',vname)
    sink(paste(vpath,'anova.out',sep="/"))
    eval(parse(text=form))
    print(paste(vname,"~(S*DID*grp)+Error(pp/(S*DID))+grp"))
    print(ez.mod)
    sink()
}

do.compare.ANOVA.did <- function(mdata,vname,vpath){       
    full.mod<-lmer(  get(vname) ~ (1|pp) + (1|pp/DID) + (1|pp/S) + (1|pp/S:DID) + S + DID + grp, mdata)
    nogrp.mod<-lmer( get(vname) ~ (1|pp) + (1|pp/DID) + (1|pp/S) + (1|pp/S:DID) + S + DID, mdata)
    noS.mod<-lmer(   get(vname) ~ (1|pp) + (1|pp/DID) + DID + grp, mdata)
    noSgrp.mod<-lmer(get(vname) ~ (1|pp) + (1|pp/DID) + DID, mdata)
...
do.lmer.did <- function(mdata,vname,vpath){
    #Linear Mixed Model effects, using lmer
    form<-sprintf('lmer.mod <- lmer( %s ~ (1|pp) + (1|pp/S) + (1|pp/DID) + (1|pp/S:DID) +  grp*S*DID, data=mdata)',vname)
...
do.lmer.tukey.did <- function(mdata,vname,vpath){
    #Linear Mixed Model effects, using lmer
    form<-sprintf('lmer.mod <- lmer( %s ~ (1|pp) + (1|pp/S) + (1|pp/DID) + (1|pp/S:DID) +  grp*S*DID, data=mdata)',vname)
    ...
    print("DID Tukey test")
    print(summary(glht(lmer.mod,linfct=mcp(DID="Tukey",interaction_average=TRUE))))
    print("S Tukey test")
