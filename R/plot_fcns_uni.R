...
plot_interactions.uni <- function(mdata,vname,vpath){
    ...
    ip2<-interaction2wt(form,responselab=vname,subset=mdata$S==2,main.in=paste(vname,"  PP vs ID, S4"),mdata)
    ip3<-interaction2wt(form,responselab=vname,subset=mdata$S==3,main.in=paste(vname,"  PP vs ID, S7"),mdata)
...
plot_boxplots.uni <- function(mdata,vname,vpath){
    ...
    #Plot group vs ID
    png(file=paste(vpath,"Uni_grp_vs_ID.png",sep="/"), bg="transparent", width=700, height=500)
    form=as.formula(paste(vname,"~ grp | ID"))
    plot(bwplot(form,mdata,main = "Group vs ID"))
    dev.off()
    
    #Plot group vs ID, 1 per session    
    form<-as.formula(paste(vname,"~ grp | ID"))
    bw1<-bwplot(form,mdata,main = "Group vs ID Session 1",subset=mdata$S==1,ylim=ylim_var)
    bw2<-bwplot(form,mdata,main = "Group vs ID Session 4",subset=mdata$S==2,ylim=ylim_var)
    bw3<-bwplot(form,mdata,main = "Group vs ID Session 7",subset=mdata$S==3,ylim=ylim_var)
    ...
    #Plot Session vs ID, 1 per group    
    form=as.formula(paste(vname,"~ S | ID"))
    ...
    bw2<-bwplot(form,mdata,main = "Participant vs ID Session 4",subset=mdata$S==2,ylim=ylim_var)
    bw3<-bwplot(form,mdata,main = "Participant vs ID Session 7",subset=mdata$S==3,ylim=ylim_var)
