function [f1mat,f2mat]=get_pairwise_comparisons(holmmat,data,factors)
    ...
    switch length(factors)
        case 1
            if verbose
                fprintf(fid,'Not necessary to do a cross level analysis\n');
            end
            f1mat=[];
            f2mat=[];
        case 2
            ...
            if verbose
                fprintf(fid,'Crosslevel interactions for factors %s vs %s\n',factors{1},factors{2});
            end
        ...
        case 3
            ...
            for g1=1:flen(1)
                if verbose
                    fprintf(fid,'Crosslevel interactions for factors %s vs %s with factor %s = %d\n',factors{2},factors{3},factors{1},g1);
                end
                for f1=1:flen(2)
                    if flen(3)==2
        ...
        case 4
            ...
            for g1=1:flen(1)
                for g2=1:flen(2)
                    if verbose
                        fprintf(fid,'Crosslevel interactions for factors %s vs %s with factor %s = %d and factor %s = %d\n',factors{3},factors{4},factors{1},g1,factors{2},g2);
                    end
                    for f1=1:flen(3)
                        if flen(4)==2
